...
/**
 * TODO update documentation Implementation of C {@code va_list} handling for System V systems on
 * AMD64 (Linux, but same behavior on Darwin). A {@code va_list} is used for passing the arguments
 * of a C varargs function ({@code ...} in the argument list) to another function. Varargs functions
 * use the same calling convention as other functions, which entails passing the first few arguments
 * in registers and the remaining arguments (if any) on the stack. Therefore, a varargs function
 * creating a {@code va_list} must initially save the contents of the argument registers so that
 * they can be read from the {@code va_list} in another function. The {@code va_list} structure
 * looks like this:
 *
 * <pre>
 *   typedef struct {
...
final class PosixAArch64VaListSnippets extends SubstrateTemplates implements Snippets {
    ...
    private static final int NUM_GP_ARG_REGISTERS = 8;
